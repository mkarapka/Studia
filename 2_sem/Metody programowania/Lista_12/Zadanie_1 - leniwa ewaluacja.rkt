...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE x e1 e2)
     (eval e2 (extend-env env x (lazyV e1 env)))]
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
...
(define (apply [v : Value] [e : Exp]) : Value
  (type-case Value v
    [(funV x b env)
     (eval b (extend-env env x (lazyV e env)))]
    [else (error 'apply "not a function")]))

 (define (force [v : Value]) : Value
     (type-case Value v
       [(lazyV e env) (eval e env)]
       [else v]))
;; $$$
...
; nie ewaluujemy odrazu, tylko wrzycamy do środowiska
; i kiedy natrafimy an zmienną dopierą ją dobliczamy