...
;$$$
;; free variables
(define (check-bounding [n : Symbol] [env : Env]) : (Listof Symbol)
  (type-case (Listof Binding) env
    [empty (list n)]
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b)) '()]
                        [else (check-bounding n rst-env)])]))

(define (fv [e : Exp] [env : Env]) : (Listof Symbol)
  (type-case Exp e
    [(numE n) '()]
    [(opE o l r) (append (fv l env) (fv r env))]
    [(ifE b l r) (append (fv b env) (append (fv l env) (fv r env)))]
    [(varE x)
     (check-bounding x env)]
    [(letE x e1 e2)
     (let ([v1 (fv e1 env)])
       (fv e2 (extend-env env x (numV 0))))]
    [(lamE x b)
     (fv b (extend-env env x (numV 0)))]
    [(appE e1 e2)
     (append (fv e1 env) (fv e2 env))]))
;$$$
;; printer ———————————————————————————————————-
