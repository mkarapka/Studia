...
;porównywanie dwóch argumentów tego samego typu 
  (define (key arg arg1)
...
(define (table-project cols tab)
  ...
  ;*przechodzenie po cols
  (define (search-name cols name)
    (cond [(empty? cols) #f]
          [(equal? (first cols) name) #t]
          [else (search-name (rest cols) name)]))
  
  ;&tworzenie podzbioru dla danego wiersza
  (define (pom cols schema row)
    (cond
      [(empty? cols) null]
      [(empty? schema) null]
      [(search-name cols (column-info-name (first schema)))
       (cons (first row) (pom cols (rest schema) (rest row)))]
      [else (pom cols (rest schema) (rest row))]))

  ;&używanie pom dla każdego wiersza
  (define (create-rows cols schema rows xs)
    (if (empty? rows) xs
    (create-rows cols schema (rest rows)
      (cons (pom cols schema (first rows)) xs))))

  ;&tworzenie podzbioru schema
  (define (create-schema cols schema)
    (cond
      [(empty? cols) null]
      [(empty? schema) null]
      [(search-name cols (column-info-name (first schema)))
       (cons (first schema) (create-schema cols (rest schema)))]
...
(define (table-select form tab)
  ...
  ;zwracanie wartości szukanje kolumny
  ...
  (define (search-column row schema name val)
    (equal? val (val-row row schema name)))

  ;porównanie dwóch wartości jeśli it-f
  (define (search-column-lt row schema name val)
    (key (val-row row schema name) val))
    ...
    (define (f-result formula)
      (cond [(and-f? formula)
            ...
            [(eq-f? formula) (search-column row schema
             (eq-f-name formula) (eq-f-val formula))]
            [(eq2-f? formula) (search-column row schema (eq2-f-name formula)
             (val-row row schema (eq2-f-name2 formula)))]
            [(lt-f? formula)
             (search-column-lt row schema (lt-f-name formula) (lt-f-val formula))]))
    (f-result formula))

  ;zwracanie wierszy na których zrobiliśmy selekcje 
...
(define (table-cross-join tab1 tab2)
  ...
  ;wykonanie złączenia kartezjańskiego na dwóch wierszach
  (define (new-rows row Rows xs)
    (if (empty? Rows) xs
        (new-rows row (rest Rows)
                  (cons (append row (first Rows)) xs))))

  ;przejście po wszystkich wierszach
  ;z tab1 i wykonanie dla nich new-rows
