...
(define (fun-parse [s : S-Exp]) : (Symbol * Exp)
   (let ([psd-fun (fE (map s-exp->symbol
     ...
     (if (all-bound?  (fE-xs psd-fun) (all-vars (fE-e psd-fun)))
         (error 'fun-parse "free variables")
         (pair (s-exp->symbol (second (s-exp->list s))) psd-fun))))
     
(define (all-bound? [fun : (Listof Symbol)]  [lst :  (Listof Symbol)]) : Boolean
  (cond
    [(empty? lst) #t]
    [(member (first lst) fun) #f]
    [else (all-bound? fun (rest lst))]))
...
(define (all-vars [e : Exp]) : (Listof Symbol)
    (type-case Exp e
      ...
      [(appE x xs) (foldr (λ (e a) (append a (all-vars e))) '() xs)]
