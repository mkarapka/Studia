...
(define (decompile [c : Code]) : Exp
  ( local [(define (pom [c : Code]) : Exp
  (type-case Instr (first c)
    [(pushI n) (numE (pushI-n (first c)))]
    [(opI op) (opE op (pom (rest (reverse (rest c)))) (pom (reverse (rest c))))]))]
     (pom (reverse c))))
; ZAD_2 koniec
...
(define wyr (opE (mul) (opE (add) (numE 3) (numE 4)) (opE (add) (numE 2) (numE 3))))
